...
7. Generate using `tts_selector` — it auto-routes to the best available TTS provider based on user preference and availability. Check the registry's `best_for` fields to understand each provider's strengths.
   - Pass `script.voice_id` (e.g. a brand's stored ElevenLabs voice) as `voice_id` so the request routes to that exact voice. If the brief is a Witers production and `script.voice_id` is not yet set (no Brand Wallet voice configured), use one of Witers' own pre-approved ElevenLabs voices from `tools/audio/witers_voice_library.py` instead of a generic default — do not search or pick a new voice from scratch.
